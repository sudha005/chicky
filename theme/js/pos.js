...
function save_vendor(percentage,cust){
print=true;
...
//$('.make_sale').click(function (e) {
	...
	//swal({ title: "Are you sure?",icon: "warning",buttons: true,dangerMode: true,}).then((sure) => {
		...
		$.ajax({
			type: 'POST',
			url: base_url+'pos/pos_save_update_vendor?command='+command+'&tot_qty='+tot_qty+'&tot_amt='+tot_amt+'&tot_disc='+tot_disc+'&tot_grand='+tot_grand+"&paid_amt="+paid_amt+'&balance='+balance+'&percentage='+percentage+'&cust='+cust,
			data: data,
			cache: false,
			contentType: false,
			processData: false,
