...
class Pos_model extends CI_Model {
	...
	public function get_details(){
		...
		extract($_POST);
		  ...
		  $table='';
	      $q2=$this->db->query("select b.id as tax_id, a.*,b.tax,b.tax_name,a.tax_type,a.item_image from db_items a,db_tax b where b.id=a.tax_id and a.status=1 $str order by a.stock desc limit 0,100");
	      if($q2->num_rows()>0){
	        foreach($q2->result() as $res2){
	          ...
	          <div class="box box-default item_box" id="div_'.$res2->id.'" onclick="'.$str.'"
	           	...
	           	<span class="label label-danger push-right" style="font-weight: bold;font-family: sans-serif;" data-toggle="tooltip" title="'.$res2->stock.' Quantity in Stock">'.$item_sales_price.'</span>
	            <div class="box-body box-profile">
	            	<center>
	            	<img class=" img-responsive item_image" style="border: 1px solid gray;"  src="'.$img_src.'" alt="Item picture">
	              </center>
	              <lable class="text-center search_item" style="font-weight: bold;font-family: sans-serif;" id="item_'.$i.'">'.substr($res2->item_name,0,25).'</label>
	            </div>
	          </div>
	        </div>';
	          $i++;
	...
	public function update_items_quantity($item_id){
		...
		$q7=$this->db->query("update db_items set stock='1500000' where id='$item_id'");
		if($q7){
			return true;
		}
		else{
	...
	public function hold_invoice_edit(){
		...
		public function get_details_quick(){
	
		  $i=0;
		  $str='';
		  
		  	$str="and a.quick_food=1";
		...
		public function get_details_modifier(){
	
		  $i=0;
		  $str='';
		  
		  	$str="and a.modifier=1";
	...
	// for vendor sales
	//Save Sales
		public function pos_save_update_vendor(){//Save or update sales
		$this->db->trans_begin();
		extract($this->xss_html_filter(array_merge($this->data,$_POST,$_GET)));
		//print_r($this->xss_html_filter(array_merge($this->data,$_POST,$_GET)));exit();
		...
		for($i=0;$i<$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && trim($_REQUEST['tr_item_id_'.$i])!=''){
				...
				$description =$this->xss_html_filter(trim($_REQUEST['description_'.$i]));
				if($percentage > 0){
				    $new_amount = ($percentage / 100) * $price_per_unit;
				   
				
			      $price_per_unit=$price_per_unit+$new_amount;
				 $total_cost=($price_per_unit + ($tax_value * $price_per_unit / 100))*$sales_qty;
				 	$subnew_total=$subnew_total+($price_per_unit**$sales_qty);
		            $subnew_grand=$subnew_grand+$total_cost;
				}else{
				    $new_amount = 0;
				}
				if($tax_type=='Exclusive'){
        ...
        $sales_entry2 = array(
'subtotal' 					=> $subnew_total,
'round_off' 				=> 0,
'grand_total' 				=> $subnew_grand,
);				
$q3 = $this->db->where('id',$sales_id)->update('db_sales', $sales_entry2);
		//UPDATE CUSTMER MULTPLE PAYMENTS
		for($i=1;$i<=$payment_row_count;$i++){
